...
async def dashboard(
    ...
    try:
        ...
        today_rev = today_revenue.scalar() or 0

        new_guests_today = await db.execute(
            select(func.count(Guest.id))
            .where(Guest.network_id == current_user.network_id, Guest.created_at >= today_start)
        )
        new_guests = new_guests_today.scalar() or 0
        ...
        orders = active_orders.scalars().all()
        ...
        top_items_data = [{"name": r.name, "qty": int(r.qty)} for r in top_items]
