...
async def test_waiter_assignment_on_pos_order(client: AsyncClient, db):
    """Order carries the waiter (staff user) who placed it, and advances
    cleanly through its status lifecycle."""
    h, venue_id, item_id = await _setup(client)
    order = (await client.post("/api/pos/order", headers=h, json={
        "venue_id": venue_id,
        "items": [{"menu_item_id": item_id, "quantity": 1}],
